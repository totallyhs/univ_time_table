...
public class CSMTimetable {
	...
	public List<Map<Integer, List<ClassVo>>> cultureCombination(Map<Integer, List<ClassVo>> checkedClassMap,
			int unitssum) {
		// 모든 교양과목 리스트
		List<ClassVo> cultureSubjectList = combinationdao.getcultivateClasses();

		// 최종 결과 시간표(리스트)의 리스트
		List<Map<Integer, List<ClassVo>>> resultList = new ArrayList();
